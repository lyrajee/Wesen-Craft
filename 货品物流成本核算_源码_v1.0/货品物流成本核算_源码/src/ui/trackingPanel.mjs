...
export function mountTrackingPanel({state,tx,getLang,onMutation}){
  ...
  function schedulePoll(){clearTimeout(timer);timer=null;if(!active||!root.closest('#tracking')?.classList.contains('on')||document.hidden)return;const tracking=repository.get(activeBatch().id,selectedId);if(!tracking||trackingLookupError(tracking.mode,tracking.identifiers)||tracking.trackingStatus==='completed'||tracking.trackingStatus==='provider_not_configured'||completeTypes.has(tracking.events?.at(-1)?.normalizedType))return;if(tracking.mode==='air'&&(tracking.status?.landed||tracking.position?.onGround))return;timer=setTimeout(()=>void refresh(tracking),tracking.mode==='air'?60000:180000);}
