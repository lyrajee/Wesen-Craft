...
const errorKey={MARINETRAFFIC_API_KEY_MISSING:'trackingSeaKeyMissing',AIRNAV_API_KEY_MISSING:'trackingAirKeyMissing',TRACKING_INVALID_IMO:'trackingInvalidImo',TRACKING_INVALID_MMSI:'trackingInvalidMmsi',TRACKING_INVALID_FLIGHT:'trackingInvalidFlight',TRACKING_IDENTIFIER_REQUIRES_IMO_OR_MMSI:'trackingIdentifierNeeded',TRACKING_NO_RESULT:'trackingNoProviderResult',PROVIDER_PERMISSION_UNAVAILABLE:'trackingPermissionUnavailable',PROVIDER_QUOTA_EXCEEDED:'trackingQuotaExceeded',PROVIDER_TIMEOUT:'trackingTimeout',PROVIDER_UNAVAILABLE:'trackingUnavailable',PROVIDER_UNAUTHORIZED:'trackingUnauthorized',PROVIDER_INVALID_RESPONSE:'trackingInvalidResponse',PROVIDER_REQUEST_FAILED:'trackingUnavailable'};
...
export function mountTrackingPanel({state,tx,getLang,onMutation}){
  ...
  function providerBanner(tracking){
    if(!tracking)return `<div class="tracking-notice info">${esc(tx(mode==='sea'?'trackingSeaKeyMissing':'trackingAirKeyMissing'))}</div>`;
  ...
  function renderCreateForm(batch){
    ...
    ].join(''):[formField('flightNo','trackingFlight'),formField('awbNo','trackingAwb'),formField('departureAirport','trackingDepartureAirport'),formField('arrivalAirport','trackingArrivalAirport'),formField('flightDate','trackingFlightDate','date')].join('');
  ...
  function render(batch=activeBatch()){
    ...
    root.querySelectorAll('[data-mode]').forEach(button=>button.onclick=()=>{mode=button.dataset.mode;selectedId=null;openForm=false;render();});
    root.querySelectorAll('[data-record]').forEach(button=>button.onclick=()=>{selectedId=button.dataset.record;lastError='';render();});
    ...
    const remove=root.querySelector('#trackingDelete');if(remove)remove.onclick=()=>{if(!globalThis.confirm(tx('trackingDeleteConfirm')))return;repository.remove(batch.id,tracking.id);selectedId=null;onMutation?.();render();};
  ...
  function create(event){
    ...
    const identifiers=formMode==='sea'?{containerNo:value('containerNo'),blNo:value('blNo'),vesselName:value('vesselName'),imo:value('imo'),mmsi:value('mmsi'),voyageNo:value('voyageNo')}:{flightNo:value('flightNo').toUpperCase(),awbNo:value('awbNo'),departureAirport:value('departureAirport').toUpperCase(),arrivalAirport:value('arrivalAirport').toUpperCase(),flightDate:value('flightDate')};
    const tracking=repository.create(batch.id,{mode:formMode,provider:formMode==='sea'?'MarineTraffic':'AirNav Radar',trackingStatus:'configured',identifiers,origin:{name:origin},destination:{name:destination},carrier:{name:carrier,vesselOrFlight:formMode==='sea'?value('vesselName'):value('flightNo'),voyageNo:value('voyageNo')},schedule:{etd:value('etd'),eta:value('eta'),etaSource:value('eta')?'Manual':null,etaUpdatedAt:value('eta')?new Date().toISOString():null,scheduledDeparture:formMode==='air'?value('etd'):null},etaHistory:value('eta')?[{eta:value('eta'),source:'Manual',observedAt:new Date().toISOString()}]:[],routeLegs:[leg],freeTimeDays:value('freeTimeDays')===''?null:Number(value('freeTimeDays')),freeTimeEndDate:value('freeTimeEndDate')||null});
    selectedId=tracking.id;openForm=false;const stored=repository.get(batch.id,tracking.id);if(stored){recomputeAlerts(stored);saveTracking(stored);}onMutation?.();render();
  }
  ...
  async function refresh(tracking){
    if(refreshing)return;refreshing=true;lastError='';render();
    try{
      ...
      if(data.identifiers?.imo)latest.identifiers.imo=data.identifiers.imo;if(data.identifiers?.mmsi)latest.identifiers.mmsi=data.identifiers.mmsi;
      ...
      latest.trackingStatus=completeTypes.has(latest.events.at(-1)?.normalizedType)?'completed':latest.position?'live':'configured';
      recomputeAlerts(latest);saveTracking(latest);
    }catch(error){
      const code=String(error?.message||'PROVIDER_UNAVAILABLE'),latest=repository.get(tracking.batchId,tracking.id);if(latest){latest.lastProviderError=code;latest.trackingStatus=errorKey[code]?.includes('KeyMissing')?'provider_not_configured':latest.lastSuccessfulUpdate?'stale':'error';recomputeAlerts(latest);saveTracking(latest);}lastError=code;
    }finally{refreshing=false;render();schedulePoll();}
  }
  function schedulePoll(){clearTimeout(timer);timer=null;if(!active||!root.closest('#tracking')?.classList.contains('on')||document.hidden)return;const tracking=repository.get(activeBatch().id,selectedId);if(!tracking||completeTypes.has(tracking.events?.at(-1)?.normalizedType))return;timer=setTimeout(()=>void refresh(tracking),tracking.mode==='air'?60000:180000);}
  function enter(){active=true;const batch=activeBatch();if(!selectedId){const recommended=seedForBatch(batch);mode=recommended.mode;const first=batch.trackings?.find(item=>item.mode===mode)||batch.trackings?.[0];selectedId=first?.id||null;}render();const tracking=repository.get(batch.id,selectedId);if(tracking&&!tracking.lastProviderUpdate&&!tracking.lastProviderError)void refresh(tracking);else schedulePoll();}
