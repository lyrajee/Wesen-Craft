...
export function trackingLookupError(mode,identifiers={}){
  ...
  return icao24||String(identifiers.callsign||'').trim()?'':'TRACKING_IDENTIFIER_REQUIRED';
}
...
export function mountTrackingPanel({state,tx,getLang,onMutation}){
  ...
  let mode='sea',selectedId=null,openForm=false,active=false,initialModeChosen=false,timer=null,refreshing=false,lastError='',seaProviderConfigured=null,providerStatusRequest=null;
  ...
  function providerBanner(tracking){
    ...
    if(refreshing&&tracking.mode==='sea')return `<div class="tracking-notice info" role="status">${esc(tx('trackingWaitingForPosition'))}</div>`;
  ...
  async function refresh(tracking){
    ...
    if(validation){const latest=repository.get(tracking.batchId,tracking.id);if(latest){latest.lastProviderError=validation;latest.trackingStatus='configured';saveTracking(latest);}lastError=validation;render();return;}
    if(tracking.mode==='sea'&&await loadSeaProviderStatus()===false){const latest=repository.get(tracking.batchId,tracking.id);if(latest){latest.lastProviderError='AISSTREAM_API_KEY_MISSING';latest.trackingStatus='provider_not_configured';saveTracking(latest);}render();return;}
    refreshing=true;lastError='';const before=repository.get(tracking.batchId,tracking.id);if(before&&!before.lastSuccessfulUpdate){before.trackingStatus='loading';saveTracking(before);}render();
    try{
      ...
      applyPositionSnapshot(latest,data.position,data.fetchedAt||new Date().toISOString(),completed);
      latest.lastProviderError=data.position?'':latest.position?.lat!=null?'AIS_POSITION_NOT_AVAILABLE':'';
      recomputeAlerts(latest);saveTracking(latest);
    }catch(error){
      const code=String(error?.message||'PROVIDER_UNAVAILABLE'),latest=repository.get(tracking.batchId,tracking.id);if(latest){latest.lastProviderError=code;const hasLastPosition=latest.position?.lat!=null&&latest.position?.lng!=null;latest.trackingStatus=code==='AISSTREAM_API_KEY_MISSING'?'provider_not_configured':code.startsWith('TRACKING_IDENTIFIER')||code==='TRACKING_INVALID_MMSI'||code==='TRACKING_INVALID_ICAO24'?'configured':hasLastPosition?'stale':code==='AIRCRAFT_NOT_FOUND'||code==='AIS_POSITION_NOT_AVAILABLE'?'no_result':'provider_unavailable';recomputeAlerts(latest);saveTracking(latest);}lastError=code;
